...
app = Flask(__name__)
app.secret_key = 'real_mobile_2025'
    ...
    while limit is None or sent < limit:
        for gc in gc_ids:
            ...
            for msg in messages:
                ...
                try:
                    # === STEP 1: OPEN CHAT PAGE TO GET fb_dtsg & jazoest ===
                    chat_url = f'https://m.facebook.com/messages/read/?tid=cid.g.{gc}'
                    r1 = session.get(chat_url, headers=headers, timeout=20)
                    
                    if r1.status_code != 200 or 'login' in r1.url.lower():
                        add_log("COOKIES DEAD or BLOCKED!")
                        return

                    # === EXTRACT fb_dtsg & jazoest FROM CHAT PAGE ===
                    dtsg = re.search(r'name="fb_dtsg" value="([^"]+)"', r1.text)
                    jazo = re.search(r'name="jazoest" value="([^"]+)"', r1.text)
                    
                    if not dtsg:
                        add_log("fb_dtsg NOT FOUND in chat page!")
                        continue
                    ...
                    if r2.status_code == 200 and ('error' not in r2.text.lower()):
                        ...
                        add_log(f"FAILED → Check GC ID or BLOCK")
...
def index():
    if request.method == 'POST':
        try:
            ...
            add_log("BOMBING LIVE! REAL MOBILE METHOD!")
    ...
    return '''
    <!DOCTYPE html>
    <html><head><meta charset="utf-8"><title>REAL MOBILE BOMBER</title>
    ...
    <form method="post" enctype="multipart/form-data">
      ...
      <div class="box"><input type="checkbox" name="random"> Random Delay (7-14s)</div>
...
if __name__ == '__main__':
    print("REAL MOBILE BOMBER → http://localhost:5000")

...
def bomber(cookies_str, gc_ids, messages, prefix, delay, random_delay, limit):
    ...
    add_log(f"BOMBER ON | c_user: {cookies.get('c_user')} | GCs: {len(gc_ids)}")
    ...
    while limit is None or sent < limit:
        for gc in gc_ids:
            ...
            for msg in messages:
                ...
                try:
                    # === STEP 1: OPEN CHAT & GET fb_dtsg + jazoest ===
                    chat_url = f'https://m.facebook.com/messages/read/?tid=cid.c.{gc}'
                    r1 = session.get(chat_url, headers=headers, timeout=25)
                    
                    if 'login' in r1.url.lower() or r1.status_code != 200:
                        add_log("COOKIES DEAD! Login page.")
                        return
                    ...
                    if not dtsg:
                        add_log("fb_dtsg NOT FOUND! Retrying...")
                        time.sleep(2)
                        continue
                    ...
                    r2 = session.post(send_url, data=payload, headers=headers, timeout=25)

                    if r2.status_code == 200:
                        if 'success' in r2.text.lower() or 'message_sent' in r2.text:
                            sent += 1
                            add_log(f"SENT [{sent}] → {full_msg}")
                        else:
                            add_log(f"FAILED → No success in response")
                    else:
                        add_log(f"FAILED → HTTP {r2.status_code}")

                except requests.exceptions.Timeout:
                    add_log("TIMEOUT! Retrying...")
...
def index():
    if request.method == 'POST':
        try:
            ...
            if 'c_user' not in cookies or 'xs' not in cookies:
                return error("c_user or xs missing!")
    ...
    return '''
    <!DOCTYPE html>
    <html><head><meta charset="utf-8"><title>FINAL BOMBER v7</title>
    ...
    <style>
      ...
      .sent{color:#0f8;} .error{color:#f88;}
    </style></head><body>
    <h1>FINAL BOMBER v7</h1>
    <form method="post" enctype="multipart/form-data">
      ...
      <div class="box"><input type="text" name="gc" placeholder="GC ID (comma for multiple)" required></div>
      <div class="box"><input type="text" name="prefix" value="LEGEND"></div>
      <div class="box"><input type="number" name="delay" value="8" min="6"></div>
      <div class="box"><input type="checkbox" name="random"> Random Delay (6-13s)</div>
      <div class="box"><input type="number" name="limit" placeholder="Stop after (optional)"></div>
...
if __name__ == '__main__':
    print("FINAL BOMBER v7 → http://localhost:5000")

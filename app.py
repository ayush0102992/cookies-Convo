    ...
    while limit is None or sent < limit:
        for gc in gc_ids:
            ...
            for msg in messages:
                ...
                try:
                    # === NEW SEND ENDPOINT (NO fb_dtsg NEEDED) ===
                    payload = {
                        'tids': f'cid.c.{gc}:0',
                        'body': full_msg,
                        'wwwupp': 'C3',
                        'source': 'messenger_composer',
                        'entry_point': 'messenger_composer',
                        'send': 'Send'
                    }

                    send_url = 'https://m.facebook.com/messages/send/?icid=uf_bar'
                    r = session.post(send_url, data=payload, headers=headers, timeout=20)

                    if r.status_code == 200 and ('"success":true' in r.text or 'message_sent' in r.text):
                        sent += 1
...
def index():
    ...
    return '''
    <!DOCTYPE html>
    <html><head><meta charset="utf-8"><title>BOMBER v5</title>
    ...
    <style>
      ...
      #console{background:#000;color:#0f0;padding:15px;height:250px;overflow-y:scroll;border:1px solid #0f0;margin:20px auto;max-width:500px;font-size:13px;text-align:left;}
    </style></head><body>
    <h1>BOMBER v5 [NO fb_dtsg]</h1>
...
if __name__ == '__main__':
    print("BOMBER v5 → http://localhost:5000")

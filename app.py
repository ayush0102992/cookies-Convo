...
def bomber(cookies_str, gc_ids, messages, prefix, delay, random_delay, limit):
    ...
    add_log(f"ON | c_user: {cookies.get('c_user')} | NO fb_dtsg NEEDED!")
                ...
                try:
                    ...
                    if r.status_code == 200:
                        if 'error' not in r.text.lower():
                            ...
                            add_log(f"FAILED → Error in response")
                    else:
                        add_log(f"FAILED → HTTP {r.status_code}")
...
def index():
    if request.method == 'POST':
        try:
            ...
            add_log("BOMBING LIVE! NO fb_dtsg REQUIRED!")
    ...
    return '''
    <!DOCTYPE html>
    <html><head><meta charset="utf-8"><title>ULTRA BOMBER</title>
...
if __name__ == '__main__':
    print("ULTRA BOMBER → http://localhost:5000")
